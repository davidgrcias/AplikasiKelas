        ...
        <!-- Simple Datatable start -->
				<div class="card-box mb-30" id = "tableka">
					<div class="pd-20" style = "display: flex; align-items: center; justify-content: space-between; padding-right: 20px; box-sizing: border-box;">
						<h4 class="text-blue h4">Data Table Books</h4>
                  ...
                  <th>Total</th>
							...
							<tbody>
                <?php
    							$userfull = query("SELECT * FROM tb_buku ORDER BY id DESC");
									$i = 1;
    						?>
    						<?php foreach($userfull as $user) : ?>
								<tr id = "<?php echo $user['id']; ?>">
									...
									<td id = "namaberita"><?php echo $user["judul"]; ?></td>
                  <td> <?php echo $user["pengarang"]; ?> </td>
                  <td>
                    <?php
                    $ry = $user["idpenerbit"];
                    $yuwe = mysqli_fetch_assoc(mysqli_query($connt, "SELECT * FROM publisher WHERE id = $ry"));
                    echo $yuwe["kodepenerbit"];
                    ?>
                  </td>
                  <td> <?php echo $yuwe["namapenerbit"]; ?> </td>
									<td> <?php echo $user["denda"]; ?> </td>
                  <td><?php echo $user["jumlahbuku"]; ?></td>
                      ...
                      ss="btn btn-link font-24 p-0 line-height-1 no-arrow dropdown-toggle" href="#" role="button" data-toggle="dropdown">
    										...
    										<a class="dropdown-item" href="editbook.php?id=<?php echo $user['id']; ?>"><i class="dw dw-edit2"></i> Edit</a>
                      ...
                      <script type="text/javascript">
          							function deleteaccountadmin(iduser){
          									$(document).ready(function(){
          											...
          											var confirmalert = confirm("Are You Sure You Want To Delete This Book?");
          											<?php else : ?>
